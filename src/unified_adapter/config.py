 ...
 class Settings(BaseSettings):
    ...
    adapter_database_url: str = Field(default="postgresql://user:password@host:5432/context_forge")
    ...
    clerk_audience: Optional[str] = Field(default=None)
     ...
     def operation_allowlist(self) -> Set[str]:
         if not self.adapter_operation_allowlist:
             return set()
         return {item.strip() for item in self.adapter_operation_allowlist.split(",") if item.strip()}
